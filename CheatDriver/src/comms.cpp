...
winternl::fnKiNmiInterruptStart pKiNmiInterruptStartOrig = 0;
...
PspRundownSingleProcess(
	...
	CR3 cr3 = { 0 };
	cr3.Flags = PsProcessDirBase(Process);
	//if (Process) {
	//	identity::ResetCache(cr3);
	//}
	int i = 0;
#ifdef INTERNAL_FACILITY
...
NTSTATUS comms::Entry(KERNEL_REQUEST* pKernelRequest)
{
	...
	case INST_HIDE:
	{
		...
		//quotaLimits.Reserved0 = 33;
		//quotaLimits.Reserved1 = 14;
		//quotaLimits.MaximumWorkingSetSize = SIZE_2_MB * 64;
		//quotaLimits.MinimumWorkingSetSize = SIZE_2_MB * 64;
		//
		//ntStatus = winternl::PspSetQuotaLimits(NtCurrentProcess(), &quotaLimits, sizeof(quotaLimits), KernelMode);
		//if (!NT_SUCCESS(ntStatus)) {
		//	DbgMsg("[DRIVER] Failed adjusting working set size for current process: 0x%x", ntStatus);
		//	break;
		//}
		//
		////ntStatus = winternl::NtLockVirtualMemory(NtCurrentProcess(), &Base, &size, 2);
		////if (!NT_SUCCESS(ntStatus)) {
		...
		//ntStatus = STATUS_SUCCESS;
		//
		//kernelRequest.memoryInfo.opSize = (DWORD64)PAGE_ALIGN(kernelRequest.memoryInfo.opSize + PAGE_SIZE - 1);
		////kernelRequest.memoryInfo.opSize += PAGE_SIZE;
		//kernelRequest.memoryInfo.opDstAddr = (DWORD64)PAGE_ALIGN(kernelRequest.memoryInfo.opDstAddr);
		//
		...
		//char* pSubstitute = (char*)cpp::kMalloc(kernelRequest.memoryInfo.opSize, PAGE_READWRITE);
		//SIZE_T ret = 0;
		//MmCopyVirtualMemory(CurrentProcess(), Base, CurrentProcess(), pSubstitute, size, KernelMode, &ret);
		//memset(pSubstitute, 0xcc, kernelRequest.memoryInfo.opSize);
		//
		...
		//
		//
		//	for (DWORD pgOffset = 0; pgOffset < (PAGE_SIZE / 8); pgOffset++) {
		//		((DWORD64*)hkSecondaryInfo.pSubstitutePage)[pgOffset] = random::Next(0, 0xffffffffffffffff);
		//	}
		//
